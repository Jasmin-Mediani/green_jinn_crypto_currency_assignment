...
const TradingPairs = () => {
  ...
  let callPairs = async () => {
    try {
      let responseFromAxios = await axios.get(
    ...
    <div>
      {pairs.length > 0 ? (
        <div>
            ...
            {pairs.map((pair, index) => {
              ...
              return (
                <button
                  ...
                  key={index}
                  onClick={(event) => {
                    setPairUrlSymbol(pair.url_symbol); /*colored(event)*/
                  }}
                >
          ...
          <div>
            {pairUrlSymbol && (
              <GJNumbersView pairUrlSymbol={pairUrlSymbol}></GJNumbersView>
            )}
            {!pairUrlSymbol && (
              <p className="text-center mt-2 fw-bolder">
                Click one of the cards above to see more data
              </p>
            )}
          </div>
        </div>
        ...
        <p className="text-center mt-5">
          Trying to fetch some data. If nothing appears, please reload this page
        </p>
      )}
    </div>
  );
